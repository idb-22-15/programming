...
class Parser {
 ...
 private:
      ...
      try {
        switch (this->tok().type) {
          ...
          case TokenType::uniont:
            defined_classes.insert(this->parse_class());
            break;
          ...
          case TokenType::boolt:
            this->parse_outer_method(defined_classes);
            break;
  ...
  std::string parse_class() {
    ...
    while (this->tok().type != TokenType::rsquirly) {  // not }
  ...
  void parse_outer_method(std::set<std::string> defined_classes) {
    ...
    if (defined_classes.find(ident_token.literal) == defined_classes.end())
      throw ExceptionWrongToken(
          ident_token, "Class `" + ident_token.literal + "` is not defined");
  }
};
