...
void output() {
	if (count != 0) {
		...
		for (int i = 0; i < count; i++) {
			printX(2);
			printf("|%5d| %13s | %3d | %9s |%12s|\n", i + 1, db[i].breed, db[i].age, db[i].color, db[i].phone);
		}
		printX(3);
	}
	else {
...
void search() {
	int choice; char s[16]; int found = 0;
...
void save() {
	...
	strcat(name, ".txt");
	fp = fopen(name, "w");
	char buffer[256] = { 0 }; int i = 0;
	for (int i = 0; i < count; i++) {
		fprintf(fp, "%s %d %s %s\n", db[i].breed, db[i].age, db[i].color, db[i].phone);
	}
	fclose(fp);
}
void load() {
	...
	if (fp == NULL) {
		...
		char buffer[256] = { 0 }; int i = 0;
		while (fscanf(fp, "%s%d%s%s", db[i].breed, &db[i].age, db[i].color, db[i].phone) != EOF) {
			i++;
		}
		count = i;
		output();
...
void printX(int tx) {
	...
	if (tx == 4) {
		printf("%c", 218);
		for (int i = 0; i < 25; i++) printf("%c", 196);
		printf("%c\n", 191);
	}
	if (tx == 5) {
		printf("%c", 192);
